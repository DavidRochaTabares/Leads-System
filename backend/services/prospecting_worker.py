...
async def execute_prospecting_session(session_id: str) -> None:
    ...
    try:
                ...
                try:
                    ...
                    if data.get('website'):
                        try:
                            ...
                            website_data = await loop.run_in_executor(executor, scraper.scrape, data['website'])
        ...
        session_repo.add_log(session_id, "Starting AI analysis...")
        ...
        session_repo.add_log(session_id, "Session completed with AI analysis")
        ...
        if miner:
            loop = asyncio.get_event_loop()
            executor = ThreadPoolExecutor(max_workers=1)
            await loop.run_in_executor(executor, miner.close)
